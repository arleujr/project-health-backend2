...
export function buildApp() {
  ...
  app.register(helmet);
  app.register(cors, { origin: env.FRONTEND_URL.split(',').map((origin) => origin.trim()), credentials: true });
  ...
  app.get('/health', async () => ({ status: 'ok', service: 'core-backend', timestamp: new Date().toISOString() }));
